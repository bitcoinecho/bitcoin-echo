...
$(TEST_SYNC): test/unit/test_sync.c src/protocol/sync.c src/protocol/download_mgr.c src/protocol/peer.c src/protocol/serialize.c src/protocol/messages.c src/consensus/chainstate.c src/consensus/utxo.c src/consensus/block.c src/consensus/tx.c src/consensus/serialize.c src/crypto/sha256.c src/platform/posix.c src/app/log.c src/node/chase.c src/storage/block_tracker.c $(TEST_UTILS_OBJ)
	$(CC) $(CFLAGS) -o $@ $^ $(LDFLAGS)
