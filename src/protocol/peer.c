...
echo_result_t peer_send_version(peer_t *peer, uint64_t our_services,
    ...
    version_msg.timestamp = (int64_t)plat_time_ms() / 1000;  /* Convert to seconds */
    ...
    size_t payload_len;
    echo_result_t result = msg_version_serialize(&version_msg, payload_buf,
    ...
    if (sent != 24) {
        peer_disconnect(peer, PEER_DISCONNECT_NETWORK_ERROR, "Failed to send version header");
        return ECHO_ERR_NETWORK;
    }
    ...
    if (sent != (int)payload_len) {
        peer_disconnect(peer, PEER_DISCONNECT_NETWORK_ERROR, "Failed to send version payload");
        return ECHO_ERR_NETWORK;
    }
...
echo_result_t peer_receive(peer_t *peer, msg_t *msg) {
    ...
    /* Must be connected */
    if (peer->state == PEER_STATE_DISCONNECTED || peer->state == PEER_STATE_DISCONNECTING) {
        return ECHO_ERR_INVALID_STATE;
    }

    /* Try to receive more data */
    size_t available = PEER_RECV_BUFFER_SIZE - peer->recv_buffer_len;
    if (available > 0) {
        int received = plat_socket_recv(peer->socket,
                                       peer->recv_buffer + peer->recv_buffer_len,
                                       available);
        if (received < 0) {
            /* Network error */
            peer_disconnect(peer, PEER_DISCONNECT_NETWORK_ERROR, "Socket receive error");
            return ECHO_ERR_NETWORK;
        } else if (received == 0) {
            /* Connection closed by peer */
            peer_disconnect(peer, PEER_DISCONNECT_PEER_CLOSED, "Connection closed by peer");
            return ECHO_ERR_NETWORK;
        } else {
    ...
    if (result != ECHO_SUCCESS) {
        peer_disconnect(peer, PEER_DISCONNECT_PROTOCOL_ERROR, "Invalid message header");
        return ECHO_ERR_PROTOCOL;
    }

    /* Validate magic bytes */
    if (header.magic != MAGIC_MAINNET) {
        peer_disconnect(peer, PEER_DISCONNECT_PROTOCOL_ERROR, "Invalid network magic");
        return ECHO_ERR_PROTOCOL;
    }
    ...
    if (computed_checksum != header.checksum) {
        peer_disconnect(peer, PEER_DISCONNECT_PROTOCOL_ERROR, "Invalid message checksum");
        return ECHO_ERR_PROTOCOL;
    }
    ...
    switch (type) {
        case MSG_VERSION:
            result = msg_version_deserialize(payload, header.length, &msg->payload.version, &consumed);
            if (result == ECHO_SUCCESS) {
                ...
                if (peer->nonce_remote == peer->nonce_local) {
                    peer_disconnect(peer, PEER_DISCONNECT_PROTOCOL_ERROR, "Self-connection detected");
                    return ECHO_ERR_PROTOCOL;
                }

                /* Check protocol version */
                if (peer->version < MIN_PROTOCOL_VERSION) {
                    peer_disconnect(peer, PEER_DISCONNECT_PROTOCOL_ERROR, "Protocol version too old");
                    return ECHO_ERR_PROTOCOL;
                }

                /* Update state based on current state */
                if (peer->state == PEER_STATE_CONNECTED) {
                    /* We received version first - send our version now */
                    peer->state = PEER_STATE_HANDSHAKE_RECV;
                } else if (peer->state == PEER_STATE_HANDSHAKE_SENT) {
                    /* We sent version, now received their version - send verack */
                    peer->state = PEER_STATE_HANDSHAKE_RECV;
                }
            }
            break;
        ...
        case MSG_PING:
            result = msg_ping_deserialize(payload, header.length, &msg->payload.ping, &consumed);
            break;

        case MSG_PONG:
            result = msg_pong_deserialize(payload, header.length, &msg->payload.pong, &consumed);
            break;

        case MSG_INV:
            result = msg_inv_deserialize(payload, header.length, &msg->payload.inv, &consumed);
            break;

        case MSG_GETDATA:
            result = msg_inv_deserialize(payload, header.length, &msg->payload.getdata, &consumed);
            break;

        case MSG_ADDR:
            result = msg_addr_deserialize(payload, header.length, &msg->payload.addr, &consumed);
            break;

        case MSG_GETHEADERS:
            result = msg_getheaders_deserialize(payload, header.length, &msg->payload.getheaders, &consumed);
            break;

        case MSG_HEADERS:
            result = msg_headers_deserialize(payload, header.length, &msg->payload.headers, &consumed);
            break;

        case MSG_BLOCK:
            result = msg_block_deserialize(payload, header.length, &msg->payload.block, &consumed);
            break;

        case MSG_TX:
            result = msg_tx_deserialize(payload, header.length, &msg->payload.tx, &consumed);
            break;

        case MSG_REJECT:
            result = msg_reject_deserialize(payload, header.length, &msg->payload.reject, &consumed);
            break;

        case MSG_FEEFILTER:
            result = msg_feefilter_deserialize(payload, header.length, &msg->payload.feefilter, &consumed);
            break;

        case MSG_SENDCMPCT:
            result = msg_sendcmpct_deserialize(payload, header.length, &msg->payload.sendcmpct, &consumed);
            break;
        ...
        case MSG_UNKNOWN:
        default:
            /* Unknown message - ignore but log command */
            result = ECHO_SUCCESS;
            break;
    }

    if (result != ECHO_SUCCESS) {
        peer_disconnect(peer, PEER_DISCONNECT_PROTOCOL_ERROR, "Failed to parse message payload");
        return ECHO_ERR_PROTOCOL;
    }
    ...
    if (peer->recv_buffer_len > 0) {
        memmove(peer->recv_buffer, peer->recv_buffer + total_size, peer->recv_buffer_len);
    }
...
echo_result_t peer_queue_message(peer_t *peer, const msg_t *msg) {
    ...
    if (peer->state != PEER_STATE_READY && peer->state != PEER_STATE_HANDSHAKE_RECV) {
        /* Special case: allow verack during handshake */
        if (!(peer->state == PEER_STATE_HANDSHAKE_RECV && msg->type == MSG_VERACK)) {
            return ECHO_ERR_INVALID_STATE;
        }
    }
 ...
 */
static echo_result_t peer_send_message_internal(peer_t *peer, const msg_t *msg) {
    uint8_t header_buf[24];
...
echo_result_t peer_send_queued(peer_t *peer) {
    ...
    while (peer->send_queue_count > 0) {
        ...
        if (result != ECHO_SUCCESS) {
            peer_disconnect(peer, PEER_DISCONNECT_NETWORK_ERROR, "Failed to send message");
            return ECHO_ERR_NETWORK;
        }
...
const char *peer_state_string(peer_state_t state) {
    switch (state) {
        case PEER_STATE_DISCONNECTED:     return "DISCONNECTED";
        case PEER_STATE_CONNECTING:       return "CONNECTING";
        case PEER_STATE_CONNECTED:        return "CONNECTED";
        case PEER_STATE_HANDSHAKE_SENT:   return "HANDSHAKE_SENT";
        case PEER_STATE_HANDSHAKE_RECV:   return "HANDSHAKE_RECV";
        case PEER_STATE_READY:            return "READY";
        case PEER_STATE_DISCONNECTING:    return "DISCONNECTING";
...
const char *peer_disconnect_reason_string(peer_disconnect_reason_t reason) {
    switch (reason) {
        case PEER_DISCONNECT_NONE:            return "NONE";
        case PEER_DISCONNECT_USER:            return "USER";
        case PEER_DISCONNECT_PROTOCOL_ERROR:  return "PROTOCOL_ERROR";
        case PEER_DISCONNECT_TIMEOUT:         return "TIMEOUT";
        case PEER_DISCONNECT_PEER_CLOSED:     return "PEER_CLOSED";
        case PEER_DISCONNECT_NETWORK_ERROR:   return "NETWORK_ERROR";
        case PEER_DISCONNECT_HANDSHAKE_FAIL:  return "HANDSHAKE_FAIL";
        case PEER_DISCONNECT_MISBEHAVING:     return "MISBEHAVING";
        default:                              return "UNKNOWN";
    }
}
