...
echo_result_t work256_sub(const work256_t *a, const work256_t *b, work256_t *result) {
    ...
    int16_t borrow = 0;
    for (int i = 0; i < 32; i++) {
        int16_t diff = (int16_t)a->bytes[i] - (int16_t)b->bytes[i] - borrow;
        if (diff < 0) {
            diff += 256;
            borrow = 1;
        } else {
...
echo_result_t work256_from_bits(uint32_t bits, work256_t *work) {
     ...
     * If target occupies bytes [0, msb_pos], then target ~= target_top * 2^((msb_pos-7)*8)
     * So work ~= 2^256 / (target_top * 2^((msb_pos-7)*8))
     *          = (2^256 / 2^((msb_pos-7)*8)) / target_top
     *          = 2^(256 - (msb_pos-7)*8) / target_top
     *          = 2^(256 - 8*msb_pos + 56) / target_top
     *          = 2^(312 - 8*msb_pos) / target_top
     *
...
echo_result_t block_delta_add_created(block_delta_t *delta,
    ...
    /* Grow array */
    outpoint_t *new_created = realloc(delta->created,
                                       (delta->created_count + 1) * sizeof(outpoint_t));
    if (new_created == NULL) {
...
echo_result_t block_delta_add_spent(block_delta_t *delta,
    ...
    /* Grow array */
    utxo_entry_t **new_spent = realloc(delta->spent,
                                        (delta->spent_count + 1) * sizeof(utxo_entry_t *));
    if (new_spent == NULL) {
...
static echo_result_t chainstate_ensure_height_capacity(chainstate_t *state,
    ...
    hash256_t *new_index = realloc(state->height_index,
                                    new_capacity * sizeof(hash256_t));
    if (new_index == NULL) {
        return ECHO_ERR_NOMEM;
    }
    ...
    for (size_t tx_idx = 0; tx_idx < tx_count; tx_idx++) {
        ...
        if (!is_coinbase) {
            for (size_t in_idx = 0; in_idx < tx->input_count; in_idx++) {
                ...
                if (delta != NULL) {
                    const utxo_entry_t *spent_entry = utxo_set_lookup(
                        state->utxo_set, outpoint);
                    if (spent_entry != NULL) {
...
bool chainstate_is_on_main_chain(const chainstate_t *state,
    ...
    /* Search height index */
    for (uint32_t h = 0; h <= state->tip.height && h < state->height_index_capacity; h++) {
...
block_index_t *block_index_map_find_best(const block_index_map_t *map) {
    ...
    for (size_t i = 0; i < map->bucket_count; i++) {
        ...
        if (best == NULL || work256_compare(&index->chainwork, &best->chainwork) > 0) {
            best = index;
        }
    }
    ...
    for (size_t i = 0; i < reorg->connect_count; i++) {
        ...
        if (delta != NULL) {
            ...
            if (to_connect->height >= state->deltas_capacity) {
                ...
                block_delta_t **new_deltas = realloc(state->deltas,
                                                      new_cap * sizeof(block_delta_t *));
                if (new_deltas == NULL) {
                    block_delta_destroy(delta);
                    return ECHO_ERR_NOMEM;
                }
