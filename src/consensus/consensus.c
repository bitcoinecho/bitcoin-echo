...
const char *consensus_error_str(consensus_error_t error) {
    if (error < sizeof(consensus_error_strings) / sizeof(consensus_error_strings[0])) {
...
echo_result_t consensus_build_validation_ctx(const consensus_engine_t *engine,
    ...
    if (height > 0) {
        ...
        for (uint32_t i = 0; i < BLOCK_MTP_WINDOW && i < height; i++) {
            ...
            if (chainstate_get_block_at_height(engine->chainstate, h, &block_hash) == ECHO_OK) {
                const block_index_t *index = consensus_lookup_block_index(engine, &block_hash);
                if (index != NULL) {
    ...
    /* Set SegWit status */
    ctx->segwit_active = (height >= CONSENSUS_SEGWIT_HEIGHT) ? ECHO_TRUE : ECHO_FALSE;
...
echo_result_t consensus_build_tx_ctx(const consensus_engine_t *engine,
    ...
    if (tx->input_count > 0) {
        ctx->utxos = calloc(tx->input_count, sizeof(utxo_info_t));
        if (ctx->utxos == NULL) {
        ...
        for (size_t i = 0; i < tx->input_count; i++) {
            ...
            /* Cast away const for the mutable array */
            utxo_info_t *info = (utxo_info_t *)&ctx->utxos[i];
            info->value = utxo->value;
            info->script_pubkey = utxo->script_pubkey;
            info->script_pubkey_len = utxo->script_len;
            info->height = utxo->height;
            info->is_coinbase = utxo->is_coinbase ? ECHO_TRUE : ECHO_FALSE;
        }
    }
...
static uint32_t get_block_height(const consensus_engine_t *engine,
    ...
    const block_index_t *parent = consensus_lookup_block_index(engine, &header->prev_hash);
    if (parent == NULL) {
...
bool consensus_validate_block(const consensus_engine_t *engine,
    ...
    if (!coinbase_validate(&block->txs[0], height, max_coinbase_value, &cb_error)) {
        if (result != NULL) {
...
bool consensus_validate_tx(const consensus_engine_t *engine,
    ...
    tx_validate_ctx_t ctx;
    echo_result_t build_result = consensus_build_tx_ctx(engine, tx, block_height, block_time, &ctx);
...
echo_result_t consensus_apply_block(consensus_engine_t *engine,
    ...
    echo_result_t add_result = consensus_add_header(engine, &block->header, &index);
...
bool consensus_would_reorg(const consensus_engine_t *engine,
    ...
    if (parent != NULL && memcmp(parent->hash.bytes, tip_index->hash.bytes, 32) == 0) {
...
static echo_result_t reorg_get_block_txs(const hash256_t *block_hash,
    ...
    echo_result_t result = ctx->get_block(block_hash, &temp_block, ctx->user_data);
    if (result != ECHO_OK) {
        return result;
    }
...
echo_result_t consensus_reorganize(consensus_engine_t *engine,
    ...
    block_index_t *new_tip_index = block_index_map_lookup(
        chainstate_get_block_index_map(engine->chainstate),
        new_tip_hash
    );
    ...
    reorg_callback_ctx_t callback_ctx = {
        .get_block = get_block,
        .user_data = user_data
    };
